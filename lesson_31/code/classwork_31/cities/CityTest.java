...
//Создать массив городов: City("Denver", 600_000);
// City("Boston", 670_000);
...
//    Есть ли в списке город по имени Chicago? Какой он имеет номер в списке?
    ...
    void testComparatorByName() {
        ...
        System.out.println(index);
        if (index > 0) {
            System.out.println("Found city: " + pattern.getName() + "on index" + index);
        } else {
            System.out.println("City : " + pattern.getName() + " wasn't found, but it could be on index" + (-index - 1));
...
//            "Salem", 690_000
    ...
    void testArrayCopy() {
        ...
        //print
        printArray(citiesCopy, "citiesCopy sorted by name");
        //binary search
    ...
    void testInsertKeepSorting() {
        ...
        //подготовили место для вставки при помощи могучего Arrays.copy
...
//    Проверьте работу метода Arrays.copyOfRange
    ...
    void testSystemArrayCopy(){
        //задача расширить массив на 2 элемента
    ...
    //проверить работу метода Array.copy.Of.Range

    @Test
    void testArraysCopyOfRange(){
        City[] citiesCopy = Arrays.copyOfRange(cities,2,3);
